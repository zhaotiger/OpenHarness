...
# ========== ohmo 导入 ==========
from ohmo.workspace import get_sessions_dir  # 获取会话目录路径


# ========== 路径辅助函数组 ==========

def get_session_dir(workspace: str | Path | None = None) -> Path:
    """获取 ohmo 会话目录路径（~/.ohmo/sessions/）"""
...
def save_session_snapshot(
    *,
    ...
    此函数会写入 3 个文件：
    1. ~/.ohmo/sessions/latest.json - 全局最新会话
    2. ~/.ohmo/sessions/latest-{token}.json - 按 session_key 的最新会话（如果提供）
    3. ~/.ohmo/sessions/session-{id}.json - 具体会话快照
    """
    # ========== 步骤1：获取会话目录和生成会话 ID ==========
    session_dir = get_session_dir(workspace)
    ...
    payload = {
        ...
        "model": model,  # 使用的模型
        "system_prompt": system_prompt,  # 完整系统提示词（包含 ohmo 人格）
...
def load_latest(workspace: str | Path | None = None) -> dict[str, Any] | None:
    """加载全局最新会话快照（~/.ohmo/sessions/latest.json）"""
...
def load_latest_for_session_key(workspace: str | Path | None, session_key: str) -> dict[str, Any] | None:
    ...
    例如：session_key = "telegram:123456:789"
          返回 ~/.ohmo/sessions/latest-a1b2c3d4e5f6.json
    """
...
class OhmoSessionBackend(SessionBackend):
    """ohmo 会话后端 - 实现 OpenHarness 的 SessionBackend 接口
