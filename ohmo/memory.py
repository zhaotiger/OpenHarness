...
# ========== ohmo 导入 ==========
from ohmo.workspace import get_memory_dir  # 获取 ~/.ohmo/memory 目录
from ohmo.workspace import get_memory_index_path  # 获取 ~/.ohmo/memory/MEMORY.md 路径
...
def list_memory_files(workspace: str | Path | None = None) -> list[Path]:
    ...
    返回排序后的文件列表，例如：
    [
        ~/.ohmo/memory/project_preferences.md,
        ~/.ohmo/memory/frequent_contacts.md,
        ~/.ohmo/memory/work_habits.md
    ]
    """
...
def add_memory_entry(workspace: str | Path | None, title: str, content: str) -> Path:
    ...
    - 文件路径: ~/.ohmo/memory/project_preferences.md
...
def load_memory_prompt(workspace: str | Path | None = None, *, max_files: int = 5) -> str | None:
    ...
    会被 prompts.py 的 build_ohmo_system_prompt() 调用。
