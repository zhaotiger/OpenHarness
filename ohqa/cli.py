"""CLI entry point for the ohqa personal-agent app."""
# ohqa 个人应用的 CLI 入口点
...
# ========== ohqa 网关相关导入 ==========
from ohqa.gateway.config import load_gateway_config, save_gateway_config  # 网关配置读写
from ohqa.gateway.models import GatewayConfig  # 网关配置数据模型
from ohqa.gateway.service import (
    OhqaGatewayService,  # 网关服务类（管理多平台消息）
...
# ========== ohqa 核心功能导入 ==========
from ohqa.memory import add_memory_entry, list_memory_files, remove_memory_entry  # 记忆管理
from ohqa.runtime import launch_ohqa_react_tui, run_ohqa_backend, run_ohqa_print_mode  # 运行时
from ohqa.session_storage import OhqaSessionBackend  # 会话持久化
...
app = typer.Typer(
    name="ohqa",
    help="ohqa: a personal-agent app built on top of OpenHarness.",
    invoke_without_command=True,  # 关键：没有子命令时也执行 main() 函数
    add_completion=False,  # 不生成 shell 自动补全脚本
)
# ========== 子命令组 ==========
memory_app = typer.Typer(name="memory", help="Manage .ohqa memory")  # 记忆管理子命令
...
gateway_app = typer.Typer(name="gateway", help="Run the ohqa gateway")  # 网关管理子命令

# ========== 注册子命令到主应用 ==========
app.add_typer(memory_app)  # ohqa memory ...
app.add_typer(soul_app)    # ohqa soul ...
app.add_typer(user_app)    # ohqa user ...
app.add_typer(gateway_app) # ohqa gateway ...
...
def _prompt_provider_profile(workspace: str | Path) -> str:
    ...
    if _can_use_questionary():
        ...
        result = questionary.select("Choose provider profile for ohqa:", choices=choices, default=default_value).ask()
        if result is None:
            raise typer.Abort()
        return str(result)
    ...
    return _select_from_menu(
        "Choose provider profile for ohqa:",
        options,
...
def _prompt_channels(existing: GatewayConfig) -> tuple[list[str], dict[str, dict]]:
    ...
    print("Configure channels for ohqa gateway:")
...
def _maybe_restart_gateway(*, cwd: str | Path, workspace: str | Path) -> None:
    ...
    if not _confirm_prompt("Gateway is running. Restart now to apply changes?", default=True):
        print("Configuration saved. Restart later with `ohqa gateway restart`.")
        return
    stop_gateway_process(cwd, workspace)
    pid = start_gateway_process(cwd, workspace)
    print(f"ohqa gateway restarted (pid={pid})")
...
def main(
    ...
    workspace: str | None = typer.Option(None, "--workspace", help="Path to the ohqa workspace (defaults to ~/.ohqa)"),
    ...
    # ========== 会话恢复选项 ==========
    resume: str | None = typer.Option(None, "--resume", help="Resume an ohqa session by id"),
    continue_session: bool = typer.Option(False, "--continue", help="Continue the latest ohqa session"),
) -> None:
    """启动 ohqa 应用或调用子命令"""

    # ========== 步骤1：检查是否调用了子命令 ==========
    if ctx.invoked_subcommand is not None:  # 例如：ohqa init, ohqa gateway start
        return  # 直接返回，让子命令处理

    # ========== 步骤2：初始化工作区 ==========
    cwd_path = str(Path(cwd).resolve())  # 规范化工作目录
    workspace_root = initialize_workspace(workspace)  # 创建 ~/.ohqa/ 和模板文件
    backend = OhqaSessionBackend(workspace_root)  # 创建会话存储后端
    ...
    if continue_session:  # --continue：恢复最新会话
        ...
        if latest is None:
            print("No previous ohqa session found in this directory.", file=sys.stderr)
            raise typer.Exit(1)
        ...
        if snapshot is None:
            print(f"ohqa session not found: {resume}", file=sys.stderr)
            raise typer.Exit(1)
    ...
    if backend_only:
        raise SystemExit(
            asyncio.run(
                run_ohqa_backend(  # 调用 OpenHarness 的后端主机
                    cwd=cwd_path,
    ...
    if print_mode is not None:
        raise SystemExit(
            asyncio.run(
                run_ohqa_print_mode(  # 运行单个提示词并退出
                    prompt=print_mode,
                    cwd=cwd_path,
                    workspace=workspace_root,
                    model=model,
    ...
    raise SystemExit(
        asyncio.run(
            launch_ohqa_react_tui(  # 启动 React 终端 UI
                cwd=cwd_path,
                workspace=workspace_root,
                model=model,
                max_turns=max_turns,
...
def init_cmd(
    ...
    """Initialize the .ohqa workspace."""
    root_path = get_workspace_root(workspace)
    already_exists = root_path.exists()
    root = initialize_workspace(root_path)
    print(f"Initialized ohqa workspace at {root}")
    if already_exists:
        print("ohqa workspace already exists.")
        if not interactive:
            print("Use `ohqa config` to update provider and channel settings.")
            return
        if not _confirm_prompt("Open configuration now?", default=True):
            print("Use `ohqa config` when you want to change provider or channel settings.")
            return
    if interactive:
...
def doctor_cmd(
    ...
    """Check .ohqa workspace and provider readiness."""
    cwd_path = str(Path(cwd).resolve())
    workspace_root = initialize_workspace(workspace)
...
def memory_list_cmd(workspace: str | None = typer.Option(None, "--workspace", help="Path to the ohqa workspace (defaults to ~/.ohqa)")) -> None:
...
def soul_show_cmd(workspace: str | None = typer.Option(None, "--workspace", help="Path to the ohqa workspace (defaults to ~/.ohqa)")) -> None:
...
def user_show_cmd(workspace: str | None = typer.Option(None, "--workspace", help="Path to the ohqa workspace (defaults to ~/.ohqa)")) -> None:
...
def gateway_run_cmd(
    ...
    """Run the ohqa gateway in the foreground."""
...
def gateway_start_cmd(
    ...
    pid = start_gateway_process(cwd, workspace)
    print(f"ohqa gateway started (pid={pid})")
...
def gateway_stop_cmd(
    ...
    if stop_gateway_process(cwd, workspace):
        print("ohqa gateway stopped.")
        return
    print("ohqa gateway is not running.")
