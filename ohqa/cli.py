...
from ohqa.gateway.config import load_gateway_config, save_gateway_config
...
from ohqa.memory import add_memory_entry, list_memory_files, remove_memory_entry
from ohqa.runtime import launch_ohqa_react_tui, run_ohqa_backend, run_ohqa_print_mode
...
memory_app = typer.Typer(name="memory", help="Manage .ohqa memory")
soul_app = typer.Typer(name="soul", help="Inspect or edit soul.md")
user_app = typer.Typer(name="user", help="Inspect or edit user.md")
gateway_app = typer.Typer(name="gateway", help="Run the ohqa gateway")
...
_WORKSPACE_HELP = "Path to the ohqa workspace (defaults to ~/.ohqa)"
...
def main(
    ...
    """Launch the ohqa app or invoke a subcommand."""
    if ctx.invoked_subcommand is not None:
        return

    cwd_path = str(Path(cwd).resolve())
    workspace_root = initialize_workspace(workspace)
    backend = OhqaSessionBackend(workspace_root)
