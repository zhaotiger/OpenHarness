...
def _ohqa_extra_roots(workspace: str | Path | None) -> tuple[tuple[str, ...], tuple[str, ...]]:
...
async def run_ohqa_backend(
    *,
    ...
    """Run the shared React backend host with ohqa workspace semantics."""
    del backend_only
    cwd_path = str(Path(cwd or Path.cwd()).resolve())
    workspace_root = initialize_workspace(workspace)
    extra_skill_dirs, extra_plugin_roots = _ohqa_extra_roots(workspace_root)
    return await run_backend_host(
        cwd=cwd_path,
        model=model,
        max_turns=max_turns,
        system_prompt=build_ohqa_system_prompt(cwd_path, workspace=workspace_root),
        active_profile=provider_profile,
        api_client=api_client,
        restore_messages=restore_messages,
        restore_tool_metadata=restore_tool_metadata,
        enforce_max_turns=max_turns is not None,
        session_backend=OhqaSessionBackend(workspace_root),
...
async def launch_ohqa_react_tui(
    *,
    ...
    """Launch the shared React terminal UI with an ohqa backend."""
    ...
    env["OPENHARNESS_FRONTEND_CONFIG"] = json.dumps(
        {
            "backend_command": build_ohqa_backend_command(
                cwd=cwd_path,
...
async def run_ohqa_print_mode(
    *,
    ...
    """Run a single ohqa prompt and print the assistant output."""
    cwd_path = str(Path(cwd or Path.cwd()).resolve())
    workspace_root = initialize_workspace(workspace)
    extra_skill_dirs, extra_plugin_roots = _ohqa_extra_roots(workspace_root)
