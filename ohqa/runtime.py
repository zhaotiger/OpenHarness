"""Runtime helpers for ohqa."""
# ohqa 运行时模块 - 提供三种运行模式：后端模式、React TUI、打印模式
...
# ========== ohqa 导入 ==========
from ohqa.prompts import build_ohqa_system_prompt  # 构建 ohqa 系统提示词
from ohqa.session_storage import OhqaSessionBackend  # ohqa 会话后端
from ohqa.workspace import initialize_workspace  # 初始化工作区


# ========== 运行模式 1：后端模式（--backend-only）⭐⭐⭐ ==========
async def run_ohqa_backend(
    *,
    ...
    return await run_backend_host(
        ...
        session_backend=OhqaSessionBackend(workspace),  # ⭐ 关键：使用 ohqa 会话后端
    )


# ========== 辅助函数：构建后端命令行 ==========
def build_ohqa_backend_command(
    *,
    ...
    返回格式：
    ["python", "-m", "ohqa", "--backend-only", "--cwd", "/path/to/project", ...]

    这个命令将被传递给 React TUI 的前端进程，前端通过 stdio 与后端通信。
    """
    command = [sys.executable, "-m", "ohqa", "--backend-only"]
...
async def launch_ohqa_react_tui(
    *,
    cwd: str | None = None,  # 当前工作目录
    workspace: str | Path | None = None,  # ohqa 工作区路径
    model: str | None = None,  # 模型名称
    ...
    └────────┬────────┘
             │ stdio
    ┌────────▼────────┐
    │  ohqa 后端       │ (python -m ohqa --backend-only)
    │  (业务逻辑层)    │
    └────────┬────────┘
             │ HTTP
    ┌────────▼────────┐
    ...
    env["OPENHARNESS_FRONTEND_CONFIG"] = json.dumps(
        {
            "backend_command": build_ohqa_backend_command(  # ⭐ 关键：后端启动命令
                cwd=cwd_path,
                workspace=workspace,
                model=model,
                max_turns=max_turns,
...
async def run_ohqa_print_mode(
    *,
    ...
    - 运行单次查询后退出
    - 适合脚本调用：ohqa -p "帮我生成代码" > output.md
    ...
    try:
        ...
        bundle = await build_runtime(
            model=model,
            max_turns=max_turns,
            system_prompt=build_ohqa_system_prompt(cwd_path, workspace=workspace),  # ⭐ ohqa 系统提示词
            active_profile=provider_profile,
            session_backend=OhqaSessionBackend(workspace),  # ⭐ ohqa 会话后端
