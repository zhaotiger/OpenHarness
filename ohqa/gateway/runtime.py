...
from ohqa.gateway.config import load_gateway_config
from ohqa.prompts import build_ohqa_system_prompt
from ohqa.session_storage import OhqaSessionBackend
from ohqa.workspace import get_plugins_dir, get_skills_dir, initialize_workspace
...
class OhqaSessionRuntimePool:
    ...
    def __init__(
        ...
        *,
        ...
        self._session_backend = OhqaSessionBackend(self._workspace)
    ...
    async def get_bundle(self, session_key: str, latest_user_prompt: str | None = None) -> RuntimeBundle:
        ...
        if bundle is not None:
            logger.info(
                "ohqa runtime reusing session session_key=%s session_id=%s prompt=%r",
                session_key,
                bundle.session_id,
        ...
        logger.info(
            "ohqa runtime creating session session_key=%s restored=%s prompt=%r",
            session_key,
            bool(snapshot),
            _content_snippet(latest_user_prompt or ""),
        )
        ...
        bundle.engine.set_system_prompt(self._runtime_system_prompt(bundle, latest_user_prompt))
        logger.info(
            "ohqa runtime started session_key=%s session_id=%s restored_messages=%s",
            session_key,
            bundle.session_id,
    ...
    async def stream_message(self, message: InboundMessage, session_key: str):
        ...
        logger.info(
            "ohqa runtime processing start channel=%s chat_id=%s session_key=%s session_id=%s content=%r",
            message.channel,
            message.chat_id,
            session_key,
            bundle.session_id,
        ...
        if parsed is not None and not message.media:
            ...
            if not remote_allowed and self._remote_admin_allowed(command):
                remote_allowed = True
                logger.warning(
                    "ohqa gateway remote administrative command accepted channel=%s chat_id=%s sender_id=%s command=%s",
                    message.channel,
                    message.chat_id,
                    message.sender_id,
                    command.name,
    ...
    async def _stream_engine_message(
        ...
        *,
        ...
        if reply:
            logger.info(
                "ohqa runtime processing complete session_key=%s session_id=%s reply=%r",
                session_key,
                bundle.session_id,
    ...
    async def _convert_stream_event(
        ...
        *,
        ...
        if isinstance(event, CompactProgressEvent):
            logger.info(
                "ohqa runtime compact progress session_key=%s session_id=%s phase=%s trigger=%s attempt=%s",
                session_key,
                bundle.session_id,
                event.phase,
                event.trigger,
        ...
        if isinstance(event, StatusEvent):
            logger.info(
                "ohqa runtime status session_key=%s session_id=%s message=%r",
                session_key,
                bundle.session_id,
        ...
        if isinstance(event, ToolExecutionStarted):
            ...
            logger.info(
                "ohqa runtime tool start session_key=%s session_id=%s tool=%s summary=%r",
                session_key,
                bundle.session_id,
                event.tool_name,
                summary,
        ...
        if isinstance(event, ToolExecutionCompleted):
            logger.info(
                "ohqa runtime tool complete session_key=%s session_id=%s tool=%s",
                session_key,
                bundle.session_id,
                event.tool_name,
            )
        ...
        if isinstance(event, ErrorEvent):
            logger.error(
                "ohqa runtime error session_key=%s session_id=%s message=%r",
                session_key,
                bundle.session_id,
                _content_snippet(event.message),
            )
    ...
    async def _save_snapshot(self, bundle: RuntimeBundle, session_key: str, user_prompt: str) -> None:
        ...
        logger.info(
            "ohqa runtime saved snapshot session_key=%s session_id=%s message_count=%s",
            session_key,
            bundle.session_id,
    ...
    async def _refresh_bundle(
        ...
        logger.info(
            "ohqa runtime refreshed session_key=%s session_id=%s message_count=%s",
            session_key,
    ...
    def _runtime_system_prompt(self, bundle: RuntimeBundle, latest_user_prompt: str | None) -> str:
        if not hasattr(bundle, "current_settings"):
            return build_ohqa_system_prompt(self._cwd, workspace=self._workspace, extra_prompt=None)
        settings = bundle.current_settings()
...
def _build_inbound_user_message(message: InboundMessage) -> ConversationMessage:
    ...
    for media_path in message.media:
        ...
        try:
            ...
            logger.exception("ohqa runtime failed to encode image attachment path=%s", media_path)
