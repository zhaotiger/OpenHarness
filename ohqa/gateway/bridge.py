...
class OhqaGatewayBridge:
    ...
    def __init__(
        ...
        *,
        bus: MessageBus,
        runtime_pool: OhqaSessionRuntimePool,
    ...
    async def run(self) -> None:
        ...
        while self._running:
            ...
            logger.info(
                "ohqa inbound received channel=%s chat_id=%s sender_id=%s session_key=%s content=%r",
                message.channel,
                message.chat_id,
                message.sender_id,
                session_key,
            ...
            task = asyncio.create_task(
                ...
                name=f"ohqa-session:{session_key}",
            )
    ...
    async def _process_message(self, message, session_key: str) -> None:
        ...
        try:
            ...
            async for update in self._runtime_pool.stream_message(message, session_key):
                ...
                if not update.text:
                    continue
                logger.info(
                    "ohqa outbound update channel=%s chat_id=%s session_key=%s kind=%s content=%r",
                    message.channel,
                    message.chat_id,
                    session_key,
                    update.kind,
            ...
            logger.info(
                "ohqa session interrupted channel=%s chat_id=%s session_key=%s reason=%s",
                message.channel,
                message.chat_id,
                session_key,
            ...
            logger.exception(
                "ohqa gateway failed to process inbound message channel=%s chat_id=%s sender_id=%s session_key=%s content=%r",
                message.channel,
                message.chat_id,
                message.sender_id,
                session_key,
        ...
        if not reply:
            logger.info(
                "ohqa inbound finished without final reply channel=%s chat_id=%s session_key=%s",
                message.channel,
                message.chat_id,
                session_key,
            )
            return
        logger.info(
            "ohqa outbound final channel=%s chat_id=%s session_key=%s content=%r",
            message.channel,
            message.chat_id,
            session_key,
